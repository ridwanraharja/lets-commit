...
const EventDetail = () => {
  ...
  // Find event by ID
  const event = featuredEvents.find(e => e.id === parseInt(eventId || '0'));
  ...
  // ... (semua function helpers tetap sama)
  ...
  const generateSessions = () => {
    ...
    for (let i = 0; i < diffDays; i++) {
      ...
      sessions.push({
        id: i + 1,
        title: `Day ${i + 1}: ${event.title} Session`,
        date: sessionDate.toISOString().split('T')[0],
        time: '14:00',
        duration: 3,
        location: event.location,
        commitmentDeposit: Math.round(event.commitmentPrice / diffDays),
        participants: Math.floor(event.participant * (0.8 + Math.random() * 0.2)),
        status: i === 0 ? 'today' : i < Math.floor(diffDays / 2) ? 'completed' : 'upcoming',
        userRegistered: true,
        userAttended: i < Math.floor(diffDays / 2) ? true : null,
        canGenerateQR: i === 0 ? true : false
      });
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'upcoming':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300">Upcoming</span>;
      case 'ON_GOING':
      case 'today':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-300">Live</span>;
      case 'completed':
      case 'FINISHED':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300">Completed</span>;
      case 'ON_SALE':
        return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300">On Sale</span>;
      default:
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 relative z-10">
        ...
        >
            ...
            <div className="lg:col-span-1">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="absolute top-4 left-4">
                  {getStatusBadge(event.StatusTags)}
                </div>
            ...
            <div className="lg:col-span-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className="flex items-center gap-3 p-3 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">{formatDateRange()}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">{event.participant}/{event.maxParticipant}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">{event.organizerName}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400">{Math.round(participantPercentage)}%</span>
                </div>
          ...
          >
            <div className="bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-blue-900/20 dark:to-cyan-900/30 rounded-2xl p-6 border border-blue-200/50 dark:border-blue-700/50">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-white">Event Price</h3>
              </div>
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">${event.eventPrice}</p>
            ...
            <div className="bg-gradient-to-br from-orange-50 to-yellow-100 dark:from-orange-900/20 dark:to-yellow-900/30 rounded-2xl p-6 border border-orange-200/50 dark:border-orange-700/50">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-semibold text-gray-900 dark:text-white">Commitment Fee</h3>
              </div>
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">${event.commitmentPrice}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">Refundable deposit</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-900/20 dark:to-emerald-900/30 rounded-2xl p-6 border border-green-200/50 dark:border-green-700/50">
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">${totalPrice}</p>
              <p className="text-sm text-gray-600 dark:text-gray-400">Initial payment required</p>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-purple-900 dark:text-purple-300 mb-2">💰 Commitment Cashback System</h4>
                <ul className="text-sm text-purple-800 dark:text-purple-300 space-y-1">
                  <li>• Attend each session → Get partial cashback immediately</li>
                  <li>• Complete all sessions → Get 100% commitment fee back</li>
                  <li>• Miss sessions → Forfeit corresponding deposit amount</li>
                  <li>• Smart contract ensures automatic and transparent payouts</li>
                </ul>
              </div>
            </div>
          </motion.div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50/50 dark:bg-gray-900/50">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Session</th>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Participants</th>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1 mb-1">
                        ...
                        <span className="text-sm text-gray-900 dark:text-white">{session.date}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Clock className="w-4 h-4 text-gray-400" />
                        <span className="text-sm text-gray-900 dark:text-white">{session.time}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{session.duration}h</td>
                    <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{session.participants}</td>
                    <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${session.commitmentDeposit}</td>
                    <td className="px-6 py-4">{getStatusBadge(session.status)}</td>
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        {userRole === 'participant' ? (
                          <>
                            {session.status === 'upcoming' && !session.userRegistered && (
                            ...
                            {session.status === 'completed' && session.userAttended && (
                              <button className="px-3 py-1.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-lg hover:opacity-90 transition-opacity flex items-center gap-1">
                                ...
                                Claim ${session.commitmentDeposit}
                              </button>
                            )}
                            {session.userRegistered && session.status !== 'completed' && (
                            ...
                            {session.status === 'completed' && !session.userAttended && (
                              <span className="text-xs text-red-600 dark:text-red-400 px-2 py-1 bg-red-100 dark:bg-red-900/20 rounded-full">Missed</span>
                            )}
                          </>
                        ) : (
                          <>
                            {session.canGenerateQR && (
                              <button
          ...
          {userRole === 'participant' ? (
            <>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      ${sessions.reduce((sum, s) => sum + s.commitmentDeposit, 0)}
                    </p>
                  </div>
              ...
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      {sessions.filter(s => s.userAttended).length}/{sessions.length}
                    </p>
                  </div>
              ...
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Available Cashback</p>
                    <p className="text-2xl font-bold text-green-600">
                      ${sessions.filter(s => s.userAttended && s.status === 'completed').reduce((sum, s) => sum + s.commitmentDeposit, 0)}
                    </p>
                  </div>
            ...
            <>
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      ${event.participant * event.eventPrice}
                    </p>
                  </div>
                  <DollarSign className="w-8 h-8 text-blue-600" />
                </div>
              ...
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Avg Attendance Rate</p>
          ...
          >
            ...
            >
              <div className="p-6 border-b border-gray-200/50 dark:border-gray-700/50 flex items-center justify-between">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Session Check-in QR</h3>
                <button 
              ...
              <div className="p-6">
                ...
                <div className="text-center">
                  ...
                  <h4 className="font-semibold text-gray-900 dark:text-white mb-2">{selectedSession.title}</h4>
          ...
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900 dark:text-white">{calculateDuration()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Total Sessions:</span>
                <span className="font-medium text-gray-900 dark:text-white">{sessions.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Session Duration:</span>
                <span className="font-medium text-gray-900 dark:text-white">3 hours each</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-2xl p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-white">{event.organizerName}</h4>
                  <div className="flex items-center gap-1">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Leading blockchain education provider with over 50+ successful events and 5,000+ satisfied learners.
              </p>
              <div className="flex gap-2">
                <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 text-xs rounded-full">Verified</span>
                <span className="px-2 py-1 bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300 text-xs rounded-full">Top Rated</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <h4 className="font-semibold text-gray-900 dark:text-white">Learning Outcomes:</h4>
