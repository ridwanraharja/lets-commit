...
export default function Testimonials() {
    ...
    const [starAnimationTrigger, setStarAnimationTrigger] = useState(0);
    ...
    // Star animation interval - every 8 seconds
    useEffect(() => {
        const starInterval = setInterval(() => {
            setStarAnimationTrigger(prev => prev + 1);
        }, 8000); // 8 seconds

        return () => clearInterval(starInterval);
    }, []);

    // Update settings on window resize without restarting animation
    useEffect(() => {
        const handleResize = () => {
    ...
    const getCurrentPosition = useCallback(() => {
        const element = document.querySelector('.testimonial-wrapper');
        if (element) {
    ...
    const startAnimation = useCallback((fromPosition = 0) => {
        if (isAnimating) return;
        
        setIsAnimating(true);
        const startPos = fromPosition;
        const duration = (originalSetWidth - Math.abs(startPos)) / pixelsPerSecond;

        controls.start({
            x: [startPos, -originalSetWidth],
            transition: {
                duration: duration,
                ease: "linear",
                onUpdate: (latest) => {
                ...
                onComplete: () => {
                    // Reset position and continue seamlessly
                    setCurrentPosition(0);
                    controls.set({ x: 0 });
                    setIsAnimating(false);
                    // Restart immediately for continuous loop
                    setTimeout(() => startAnimation(0), 0);
                }
            }
        });
    }, [originalSetWidth, pixelsPerSecond, controls, isAnimating]);
    ...
    // Handle settings change without restarting animation
    useEffect(() => {
        if (isAnimating) {
            // Get current position and continue with new settings
    ...
    }, [pixelsPerSecond, getCurrentPosition, startAnimation, isAnimating]);
    ...
    }, [startAnimation, getCurrentPosition, isAnimating]);
                            ...
                            {duplicatedTestimonials.map((testimonial, index) => {
                                ...
                                return (
                                    <TestimonialCard
                                        ...
                                        index={index}
                                        cardWidth={cardWidth}
                                        starAnimationTrigger={starAnimationTrigger}
                                    />
                                );
                            })}
                        </motion.div>
                    ...
                    >
                        <div className="inline-flex items-center gap-2 md:gap-3 px-4 md:px-6 py-2 md:py-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full shadow-lg border border-gray-200/50 dark:border-gray-700/50">
                            ...
                            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400 font-medium">
                                Moving at {pixelsPerSecond}px/sec - Stars animate every 8s
                            </p>
                        </div>
                    </motion.div>
                </div>
...
// Optimized testimonial card component with timed star animation
const TestimonialCard = React.memo<TestimonialCardProps>(({ testimonial, cardWidth, starAnimationTrigger }) => {
                    ...
                    <div className="flex items-center mb-5">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                            {[...Array(testimonial.rating)].map((_, i) => (
                                <motion.div
                                    key={`${i}-${starAnimationTrigger}`}
                                    initial={{ scale: 0, rotate: -180 }}
                                    animate={isInView ? { 
                                        scale: 1, 
                                        rotate: 0,
                                        // Trigger animation every 8 seconds
                                        rotateY: starAnimationTrigger > 0 ? [0, 360, 0] : 0
                                    } : { scale: 0, rotate: -180 }}
                                    transition={{ 
                                        duration: starAnimationTrigger > 0 && isInView ? 0.6 : 0.25,
